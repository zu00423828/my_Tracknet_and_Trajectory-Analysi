...
for i in range(1):
	...
	images = glob.glob( images_path + "25_1" + "/*.jpg" )
	images.sort()


	#create folder for saving output image  
	if not os.path.exists(output_path + "25_1" + "/"):
	    os.makedirs(output_path + "25_1" + "/")
	tStart = time.time()
	#predict each images
	...
	for i in range(2,len(images)):
		...
		output_img = cv2.resize(pr  , (output_width , output_height ))
		#output heatmap image
		#cv2.imwrite(  output_name , output_img )
		#heatmap = cv2.resize(output_img  , (output_width, output_height ))
		circles = cv2.HoughCircles(output_img, cv2.HOUGH_GRADIENT,dp=1,minDist=1,param1=50,param2=2,minRadius=2,maxRadius=7)
	tEnd = time.time()
